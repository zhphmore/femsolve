...
def mesh_C2D4(geometry, num_elem_horizon, num_elem_vertical):
    ...
    # Number the nodes: from bottom to top, from left to right
    for i in range(num_node_horizon):
        for j in range(num_node_vertical):
            id_node = i * num_node_vertical + j
    ...
    for i in range(num_elem_horizon):
        for j in range(num_elem_vertical):
            id_element = i * num_elem_vertical + j
            elem[id_element, 0] = i * num_node_vertical + j
            elem[id_element, 1] = (i + 1) * num_node_vertical + j
            elem[id_element, 2] = (i + 1) * num_node_vertical + j + 1
            elem[id_element, 3] = i * num_node_vertical + j + 1
